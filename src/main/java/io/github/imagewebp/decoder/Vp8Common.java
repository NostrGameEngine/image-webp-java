...
/** VP8 constants/tables (generated from Rust src/vp8_common.rs). */
final class Vp8Common {
    private Vp8Common() {}
    ...
    static final int[] KEYFRAME_YMODE_TREE = {-B_PRED, 2, 4, 6, -DC_PRED, -V_PRED, -H_PRED, -TM_PRED};
    ...
    static final int[] KEYFRAME_UV_MODE_TREE = {-DC_PRED, 2, -V_PRED, 4, -H_PRED, -TM_PRED};

    static final short[] KEYFRAME_YMODE_PROBS = {145, 156, 163, 128};
    static final short[][][] KEYFRAME_BPRED_MODE_PROBS = {
    {
    ...
    static final short[] KEYFRAME_UV_MODE_PROBS = {142, 114, 183};

    static final short[][][][] COEFF_UPDATE_PROBS = {
    {
        {
    ...
    static final short[][][][] COEFF_PROBS = {
    {
        {
    ...
    static Vp8TreeNode[] treeNodesFrom(int[] tree, short[] probs) {
        int m = probs.length;
        if (tree.length != 2 * m) {
            throw new IllegalArgumentException("Invalid tree/probs length");
        }
        Vp8TreeNode[] nodes = new Vp8TreeNode[m];
        for (int i = 0; i < m; i++) {
            int left = Vp8TreeNode.prepareBranch(tree[2 * i]);
            int right = Vp8TreeNode.prepareBranch(tree[2 * i + 1]);
            nodes[i] = new Vp8TreeNode(left, right, probs[i] & 0xFF, i);
        }
        return nodes;
    }

    static final Vp8TreeNode[] SEGMENT_TREE_NODE_DEFAULTS = treeNodesFrom(SEGMENT_ID_TREE, new short[] {255, 255, 255});
    static final Vp8TreeNode[] KEYFRAME_YMODE_NODES = treeNodesFrom(KEYFRAME_YMODE_TREE, KEYFRAME_YMODE_PROBS);
    static final Vp8TreeNode[] KEYFRAME_UV_MODE_NODES = treeNodesFrom(KEYFRAME_UV_MODE_TREE, KEYFRAME_UV_MODE_PROBS);

    static final Vp8TreeNode[][][] KEYFRAME_BPRED_MODE_NODES = createBpredModeNodes();
    private static Vp8TreeNode[][][] createBpredModeNodes() {
        Vp8TreeNode[][][] out = new Vp8TreeNode[10][10][];
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                out[i][j] = treeNodesFrom(KEYFRAME_BPRED_MODE_TREE, KEYFRAME_BPRED_MODE_PROBS[i][j]);
            }
        }
        return out;
    }
    ...
    static Vp8TreeNode[][][][] newDefaultTokenProbNodes() {
        ...
        for (int p = 0; p < 4; p++) {
            for (int b = 0; b < 8; b++) {
                for (int c = 0; c < 3; c++) {
                    out[p][b][c] = treeNodesFrom(DCT_TOKEN_TREE, COEFF_PROBS[p][b][c]);
                }
            }
        }
        return out;
    ...
    static Vp8TreeNode[][][][] deepCopyTokenProbNodes(Vp8TreeNode[][][][] in) {
        ...
        for (int p = 0; p < 4; p++) {
            for (int b = 0; b < 8; b++) {
                for (int c = 0; c < 3; c++) {
                    Vp8TreeNode[] srcNodes = in[p][b][c];
                    Vp8TreeNode[] dstNodes = new Vp8TreeNode[srcNodes.length];
                    for (int i = 0; i < srcNodes.length; i++) {
                        Vp8TreeNode n = srcNodes[i];
                        dstNodes[i] = new Vp8TreeNode(n.left, n.right, n.prob, n.index);
                    }
                    out[p][b][c] = dstNodes;
                }
            }
