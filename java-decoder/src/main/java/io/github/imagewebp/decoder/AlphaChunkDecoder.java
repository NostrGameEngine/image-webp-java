...
final class AlphaChunkDecoder {
    ...
    static void applyAlph(byte[] webp, int alphOff, int alphLen, int width, int height, byte[] rgba)
        ...
        if (compression == 0) {
            ...
            Vp8LDecoder.decodeToRgba(webp, payloadOff, payloadLen, width, height, true, tmp);
            alpha = new byte[width * height];
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                rgba[idx * 4 + 3] = (byte) ((pred + (alpha[idx] & 0xFF)) & 0xFF);
            }
        }
    }
}
